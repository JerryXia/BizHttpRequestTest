...
public class RequestCaptureWebServlet extends AbstractResourceServlet {
    ...
    protected String process(String url) {
        ...
        if ("/alllogs.json".equals(url)) {
            List<LogEntry> alllogs = RequestCaptureConstants.RECORD_MANAGER.currentLogEntryManager().currentLogEntryStorage().queryAll();
            resp = returnJSONResult(RESULT_CODE_SUCCESS, alllogs, getServerStat(startTime));
        }
        if ("/apirecords.json".equals(url)) {
            List<HttpRequestRecord> allRecords = RequestCaptureConstants.RECORD_MANAGER.currentHttpRequestRecordStorage().queryAll();
